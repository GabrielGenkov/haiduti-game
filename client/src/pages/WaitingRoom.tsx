...
export default function WaitingRoom() {
  ...
  // Get session cookie for WS auth
  const { data: me } = trpc.auth.me.useQuery();

  // Connect WS and authenticate when we have a session
  useEffect(() => {
    if (me && !authenticated) {
      // The session cookie is httpOnly so we can't read it directly.
      // Instead we pass a special "cookie-auth" signal and the server reads the cookie from the upgrade request.
      authenticate('cookie');
    }
  ...
  const { data: roomData } = trpc.rooms.get.useQuery(
    { code },
    { enabled: !!code, refetchInterval: 3000 }
  );
    ...
    <div className="min-h-screen flex flex-col items-center justify-center px-4" style={{ background: 'oklch(0.17 0.025 55)' }}>
      ...
      >
                  ...
                  <div className="flex-1">
                    <div className="font-cinzel text-sm font-semibold" style={{ color: 'oklch(0.88 0.04 80)' }}>
                      ...
                      {player.userId === roomData?.room?.hostId && (
                        <span className="ml-2 text-xs text-amber-400">👑 Домакин</span>
                      )}
                    </div>
                  </div>
        ...
        <div className="flex gap-3">
          <button
            onClick={() => {
              if (roomData?.room?.id) leaveMutation.mutate({ roomId: roomData.room.id });
              else navigate('/lobby');
            }}
