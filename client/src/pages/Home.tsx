...
const CONTRIBUTION_ICONS: Record<ContributionType, { icon: string; label: string; color: string }> = {
  nabor:   { icon: '🎴', label: 'Набор',      color: 'text-sky-400' },
  deynost: { icon: '⚡', label: 'Дейност',    color: 'text-amber-400' },
  boyna:   { icon: '⚔️', label: 'Бойна мощ', color: 'text-red-400' },
};

function CardDisplay({ card, showBack = false }: { card: Card; showBack?: boolean }) {
        ...
        <div className={`card-face w-full h-full rounded-lg border-2 ${color?.border ?? 'border-amber-700'} ${color?.bg ?? 'bg-amber-900/30'} overflow-hidden shadow-lg`}>
          <div className="h-full flex flex-col p-2">
            ...
            <div className="flex items-center justify-between mb-1">
              ...
              {card.cost && (
                <span className="font-cinzel text-xs font-bold text-amber-300 bg-black/40 rounded px-1">💰{card.cost}</span>
              )}
            </div>
            
            {/* Illustration area */}
            <div className="flex-1 bg-black/20 rounded flex items-center justify-center text-4xl mb-1">
              {card.type === 'haydut' ? '🗡️' : card.type === 'voyvoda' ? '🏴' : '📜'}
            </div>
            ...
            <div className="flex items-center justify-center gap-1">
              ...
              {card.chetaPoints !== undefined && card.chetaPoints > 0 && (
                <span className="text-xs text-amber-400">🏳️{card.chetaPoints}</span>
              )}
            </div>
  ...
  return (
    <div className="min-h-screen" style={{ background: 'oklch(0.17 0.025 55)' }}>
      ...
      <footer className="text-center py-8 border-t" style={{ borderColor: 'oklch(0.25 0.03 55)', color: 'oklch(0.45 0.03 65)' }}>
        <p className="font-source text-sm">Хайдути © Александър Торофиев · Издава Българска История</p>
      </footer>
    </div>
  );
}
